...
graph TB
    subgraph Frontend["Frontend (React 18)"]
        FE["TypeScript · Tailwind · Radix UI · Zustand"]
    end

    subgraph Backend["Backend (Django REST Framework)"]
        ACC["accounts<br/>(auth)"]
        ART["artifacts<br/>(storage)"]
        GEN["generation<br/>(bullets)"]
        EXP["export<br/>(PDF/DOCX)"]

        subgraph LLM["llm_services (Unified LLM Layer)"]
            direction TB
            BASE["base/<br/>base_service · client_manager<br/>task_executor · exception_handler"]
            CORE["core/<br/>tailored_content · document_loader<br/>artifact_enrichment · artifact_ranking"]
            INFRA["infrastructure/<br/>model_registry · model_selector"]
            REL["reliability/<br/>circuit_breaker · perf_tracker"]
        end
    ...
    Backend --> RD["Redis"]
    Backend --> CL["Celery Workers"]

    style Frontend fill:#e0f2fe,stroke:#0284c7
    style Backend fill:#f8fafc,stroke:#64748b
